...
var whichConfig = function(config, process){
  ...
  var configFileName      = '/phantomflier.json'
  ,   localConfigFile     = process.cwd() + configFileName
  ,   localConfigExists   = fs.existsSync(localConfigFile)
  ,   configParam         = config.options.config
  ,   hasConfigParam      = _.has(config.options.config)
  ,   paramConfigExists   = fs.existsSync(configParam);
  ...
  else if( hasConfigParam && !paramConfigExists){
